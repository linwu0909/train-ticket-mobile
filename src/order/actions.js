...
export function removePassenger(id) {
    return (dispatch, getState) => {
        ...
        const newPassengers = passengers.filter(passenger => {
            return passenger.id !== id && passenger.followAudit !== id;
        });
...
export function showFollowAdultMenu(id) {
    return (dispatch, getState) => {
        const { passengers } = getState();
        const passenger = passengers.find(passenger => passenger.id);
        if (!passenger) {
            return;
        }
        dispatch(
            showMenu({
                onPress(followAudit) {
                    dispatch(updatePassenger(id, { followAudit }));
                    dispatch(hideMenu());
                },
                options: passengers
                    ...
                    .map(adult => {
                        return {
                            title: adult.name,
                            value: adult.id,
                            active: adult.id === passenger.followAudit,
                        };
                    }),
            })
        );
